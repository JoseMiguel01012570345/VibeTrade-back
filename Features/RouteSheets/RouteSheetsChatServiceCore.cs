...
public sealed class RouteSheetsChatServiceCore(
    AppDbContext db,
    IChatService chat,
    ITrustScoreLedgerService trustLedger,
    IDrivingLegRoutingService drivingLegRouting,
    ILogger<RouteSheetsChatServiceCore> logger,
    ...
    ICatalogSearchLiveIndexSync catalogSearchLiveIndex)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (payload.PublicadaPlataforma == true)
        {
            ...
            var linkedToAgreement = await db.TradeAgreements.AsNoTracking()
                .AnyAsync(
            ...
            if (!linkedToAgreement)
                return new UpsertLoadResult(RouteSheetMutationResult.CannotPublishWithoutAgreementLink, null);
        }
    ...
    public async Task<RouteSheetPayload?> ComputeRoutePathAsync(
        RouteSheetPayload payload,
        CancellationToken cancellationToken = default)
    {
        var persisted = RouteSheetUtils.ClonePayload(payload);
        await RouteSheetOsrmRoadKmPopulator.ApplyAsync(persisted, drivingLegRouting, logger, cancellationToken);
        if (RouteSheetUtils.ValidateEstimatedTimes(persisted) is not null)
            return null;
        return persisted;
