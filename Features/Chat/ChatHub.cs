...
public sealed class ChatHub(IAuthService auth, IServiceScopeFactory scopeFactory) : Hub
{
    ...
    /// <summary>Aviso explícito «salir»: notifica al resto (vía <see cref="IBroadcastingService.BroadcastParticipantLeftToOthersAsync"/>) y deja el grupo del hilo.</summary>
    public async Task NotifyOthersUserLeftChat(string threadId)
    {
        ...
        await using var scope = scopeFactory.CreateAsyncScope();
        var broadcasting = scope.ServiceProvider.GetRequiredService<IBroadcastingService>();
        if (!await broadcasting.BroadcastParticipantLeftToOthersAsync(userId, tid, Context.ConnectionAborted))
            return;
